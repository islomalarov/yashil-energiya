...
const totalPvPlants = plants.reduce((total, plant) => total + plant.plants, 0);

export default function SolarPanels() {
  const locale = useLocale();
  const t = useTranslations("SolarPanelsPage");
  const formattedTotalPvPlants = new Intl.NumberFormat(locale).format(totalPvPlants);
